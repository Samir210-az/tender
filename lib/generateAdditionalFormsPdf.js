  ...
  return new Promise((resolve, reject) => {
    ...
    if (forma4Sections?.length !== undefined) {
      ...
      if (forma4Sections.length > 0) {
        doc.font(fontRegular).fontSize(10).fillColor('#333333').text(`Təchizatçının adı: ${forma4Sections[0].companyName}`);
      }
      doc.moveDown(0.4);
      if (forma4Sections.length === 0) {
        ...
        forma4Sections.forEach((s, idx) => {
          ...
          doc.moveDown(0.2);
          doc.text(`Namizədin adı: ${s.fullName}  İmza: _______________  Tarix: _________`);
