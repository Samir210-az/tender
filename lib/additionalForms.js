 ...
 * siyahısından, deterministik.
 */
export function buildForma4Sections({ employees }) {
  return (employees || []).map((emp) => ({
    ...
    lines: [
      ...
      `Telefon: ${emp.phone || '—'}`,
      `E-poçt: ${emp.email || '—'}`,
      `Təhsil: ${emp.education || '—'}`,
      `Dil bilikləri: ${emp.languages || '—'}`,
      `Peşəkar sertifikatlar: ${emp.professional_certificates || '—'}`,
      `İş təcrübəsi: ${emp.work_experience || '—'}`,
    ],
  }));
}
