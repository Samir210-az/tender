...
export function buildForma5Lines({ profile }) {
  return [
    ...
    `  Ünvanı: ${profile.authorized_rep_address || '(doldurulmayıb)'}`,
 ...
 */
export function buildForma4Sections({ employees, companyName, companyAddress, contactPersonName }) {
  return (employees || []).map((emp) => ({
    ...
    employerLines: [
      ...
      `İşəgötürənin ünvanı: ${companyAddress || '—'}`,
      `Əlaqəli şəxs: ${contactPersonName || '—'}`,
      `Vəzifəsi: ${emp.position || '—'}`,
      `Hazırkı iş yerində çalışma müddəti: (izlənilmir — əl ilə əlavə edilməlidir)`,
    ],
