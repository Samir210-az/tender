 ...
 */
export async function generateProposalPdf({ tenderName, sections, addresseeLines, signatureLines, formaOneDeclaration }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (formaOneDeclaration?.length) {
      doc.font(fontBold).fontSize(13).fillColor('#000000').text('Bəyanat (FORMA 1)');
      doc.moveDown(0.4);
      doc.font(fontRegular).fontSize(10).fillColor('#222222');
      for (const line of formaOneDeclaration) {
    ...
    if (signatureLines?.length) {
      ...
      for (const line of signatureLines) doc.text(line);
    }
