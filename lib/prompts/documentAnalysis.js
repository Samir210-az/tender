...
8. Əgər prompt-da "HÜQUQİ KONTEKST" bölməsi varsa, bu YALNIZ sənə kateqoriyalaşdırma üçün fon məlumatıdır — SƏNƏDİN BİR HİSSƏSİ DEYİL. Hüquqi kontekstdən heç bir tələb çıxarma, heç bir "source_excerpt" yaratma. Yalnız "SƏNƏD MƏTNİ" bölməsindəki mətndən tələb çıxar. Hüquqi kontekstdəki hər hansı ifadəni sənəddən sitat kimi göstərmək CİDDİ QAYDA POZUNTUSUDUR.
9. "title" və "description" sahələri HƏMİŞƏ Azərbaycan dilində olmalıdır (sənəd özü başqa dildə olsa belə) — istisna: "source_excerpt" sənədin öz orijinal dilində, dəyişdirilmədən qalmalıdır.
...
export function buildDocumentAnalysisUserPrompt(documentText, fileName, jurisdiction) {
  // Çox uzun sənədlər üçün limit (Groq-un openai/gpt-oss-120b modeli 8000 TPM
  // limitinə malikdir — system prompt + hüquqi kontekst + max_tokens ilə
  // birlikdə bu limitin altında qalmaq üçün mətn ciddi məhdudlaşdırılır).
  ...
  const legalBlock = legalContext
    ? `\n=== HÜQUQİ KONTEKST (sənədin xaricindədir, YALNIZ fon məlumatı — bu mətndən HEÇ BİR tələb çıxarma) ===\n${legalContext}\n=== HÜQUQİ KONTEKST SONU ===\n`
    : '';
