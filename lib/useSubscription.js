...
export function useSubscription() {
  ...
  const register = useCallback(async ({ companyName, phone, pin, plan }) => {
    ...
    // Client UUID-ni özü yaradır — RETURNING (.select()) çağırılmır, çünki
    // Postgres RLS-də INSERT...RETURNING əlavə SELECT policy tələb edir və
    // biz təhlükəsizlik səbəbindən anon üçün blanket SELECT icazəsi vermirik.
    ...
    const result = { id: newId, status: 'pending', plan, company_name: companyName };
    writeLocal({ regId: newId, lastCheckedAt: 0 });
    setStatus('pending');
